...
import { standingCommittees } from '@/lib/data/committee'

const shortCommitteeName = (term: string) =>
  term.replace(/\s*\(2026-2028\)\s*$/, '').trim()

const StandingCommittees = () => {
  const [selectedTerm, setSelectedTerm] = useState<string | null>(
    standingCommittees.length > 0 ? standingCommittees[0].committee_term : null
  )

  const selectedCommittee = standingCommittees.find(
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <div className="inline-flex items-center space-x-2 bg-primary-100 text-primary px-4 py-2 rounded-full text-sm font-medium mb-6">
              ...
              <span>Standing Committees (2026–2028)</span>
            </div>

            <h1 className="text-4xl sm:text-5xl font-bold text-gray-900 mb-6">
              Our <span className="text-primary">Standing Committees</span>
            </h1>

            <p className="text-xl text-gray-600 max-w-3xl mx-auto mb-8">
              Meet the members who lead Gulshan Society&apos;s standing committees
              and support the day-to-day work of our community.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-4">
              Select a <span className="text-primary">Committee</span>
            </h2>
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              Choose a standing committee to view its chairman, officers, and members.
            </p>
          </div>
          ...
          <div className="mb-12">
            <div className="flex flex-wrap justify-center gap-3">
              {standingCommittees.map((committee) => (
                ...
                >
                  {shortCommitteeName(committee.committee_term)}
                </button>
              ))}
            </div>
          </div>
          ...
          {!selectedCommittee && (
            <div className="text-center py-12">
              <p className="text-gray-600">No standing committee data available</p>
            </div>
          )}
        </div>
      </section>
